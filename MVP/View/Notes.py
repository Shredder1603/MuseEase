...
class SoundGenerator:
    ...
    '''
    def __init__(self, preloaded_samples=None):
        import soundfile as sf
        self.sf = sf
        self.mutex = QMutex()
        self.active_notes = {}
        self.notes = preloaded_samples["Piano"] if preloaded_samples and "Piano" in preloaded_samples else {}
        self.samplerate = 44100  # Match DAW's sample rate if preloaded, otherwise set later
        
        if not self.notes:  # If no preloaded samples, load them
            self.read_notes()
        
        self.stream = sd.OutputStream(
            samplerate=self.samplerate if self.samplerate else self.notes["A0"][1],
            channels=1,
    ...
    def note_on(self, note: str, loop: bool = False):
        ...
        '''
        if note not in self.active_notes and note in self.notes:
            data = self.notes[note]['data'] if isinstance(self.notes[note], dict) else self.notes[note][0]
            samplerate = self.notes[note]['sample_rate'] if isinstance(self.notes[note], dict) else self.notes[note][1]
...
class NotesWindow(QMainWindow):
    ...
    def __init__(self, sound_generator=None):
        ...
        self.sound = sound_generator if sound_generator else SoundGenerator()  # Use provided or create new
